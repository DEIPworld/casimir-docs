...
const absSrcDirs = env.opts._.map((iSrcDir) => path.resolve(env.pwd, iSrcDir));
...
function jsdocCommentFound(e) {
  ...
  e.comment = e.comment.replace(typeRegex, (typeExpr) => typeExpr.replace(identifiers, (identifier) => ((fileInfo.moduleId && typeDefsSet.has(identifier))
