...
const appendComment = (commentBlock, toAppend) => commentBlock.replace(/[\n,\s]*\*\//, `${toAppend.split('\n').map((line) => `\n * ${line}`)}\n */`);
...
const convertParams = (jsDoc = '', node, src) => {
  ...
  parameters.forEach((parameter) => {
    ...
    const comment = getCommentAsString(parameter, src);
    if (parameter.questionToken) {
      name = ['[', name, ']'].join('');
    }
    const type = getTypeName(parameter.type, src);
...
let convertMembers = (jsDoc = '', type, src, parentName = null) => {
  ...
  typesToCheck.forEach((type) => {
    ...
    (type.members || []).filter((m) => ts.isTypeElement(m)).forEach((member) => {
      ...
      const comment = getCommentAsString(member, src);
      const members = member.type.members || [];
      const typeName = members.length ? 'object' : getTypeName(member.type, src);
      if (parentName) {
...
module.exports = function typeConverter(src, filename = 'test.ts') {
  ...
  return ast.statements.map((statement) => {
    const jsDocNode = statement.jsDoc && statement.jsDoc[0];
    ...
    if (jsDocNode) {
      ...
      if (ts.isTypeAliasDeclaration(statement)) {
        ...
        if (
          ts.isUnionTypeNode(statement.type)
            || ts.isSimpleInlineableExpression(statement.type)
      ...
      if (ts.isInterfaceDeclaration(statement)) {
        ...
        statement.members.forEach((member) => {
          ...
          const memberName = getName(member, src);
          ...
          if (!member.type && ts.isFunctionLike(member)) {
            const type = getTypeName(member, src);
      ...
      if (ts.isClassDeclaration(statement)) {
        ...
        statement.members.forEach((member) => {
          ...
          } else if (modifiers.find((m) => m === 'static')) {
      ...
      //   return appendComment(comment, `@enum`)
      // }
